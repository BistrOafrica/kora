...
func executeUpdateDoctypeDraft(tx *orm.TxManager, reg *doctype.Registry, yamlStr, owner, siteName string) string {
	...
	if err != nil {
		return fmt.Sprintf("Error building draft snapshot: %v", err)
	}
	if owner == "" || owner == "mcp-agent" {
		owner = "ai-assistant"
	}
	verID, verNum, err := store.CreateConfigVersionWithBase(
		siteName, owner, "Updated "+dt.Name+" via AI (Draft)", "Draft", snapshot, baseVersionID,
	)
	if err != nil {
...
func executeCreateDoctypeDraft(tx *orm.TxManager, reg *doctype.Registry, yamlStr, owner, siteName string) string {
	...
	verID, verNum, err := store.CreateConfigVersionWithBase(
		siteName, owner, "Created "+dt.Name+" via AI (Draft)", "Draft", snapshot, baseVersionID,
	)
	if err != nil {
		slog.Warn("config version creation failed", "error", err)
	}
